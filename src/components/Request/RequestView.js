...
const RequestView = ({ history }) => {
  ...
  useEffect(() => {
    validateToken(false).then(response => {
      if (response) {
        // console.log('RequestView: effect handleDataLoad...');
        handleDataLoad();
      } else {
          ...
          .then(response => {
            ...
            if (!response.ok) {
              ...
              return response.json().then(data => {
                ...
                setSnackState({ severity: 'info', message: 'Request fetched', show: true, duration: 3000 });
              });
            }
          }).catch(error => {
    ...
    <>
      <Container maxWidth="md">
        <Box my={{ xs: 2, md: 3 }} display="flex" flexWrap="nowrap" alignItems="center">
          <Box flexGrow={1}>
            <Typography className={classes.header} variant="h6">Request View</Typography>
          </Box>
          <Button
            variant="outlined"
